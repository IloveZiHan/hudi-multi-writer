...
import { UpdateApplicationRequest, ApplicationDTO } from '@services/applicationApi';
...
const EditApplicationModal: React.FC<EditApplicationModalProps> = ({
  ...
  // 提交请求
  const { loading, run: updateApplication } = useRequest(
    (id: number, request: UpdateApplicationRequest) => 
      applicationApiService.updateApplication(id, request),
    {
      manual: true,
      onSuccess: () => {
  ...
  // 解析配置 JSON
  const parseConfig = (confStr?: string) => {
    if (!confStr) return DEFAULT_SPARK_CONFIG;
    
    try {
      const config = JSON.parse(confStr);
      return { ...DEFAULT_SPARK_CONFIG, ...config };
    } catch (error) {
      console.warn('配置解析失败，使用默认配置:', error);
  ...
  useEffect(() => {
    if (visible && record) {
      const sparkConfig = parseConfig(record.conf);
      
      form.setFieldsValue({
        name: record.name,
        description: record.description || '',
        conf: record.conf || JSON.stringify(DEFAULT_SPARK_CONFIG, null, 2),
        
        // Spark Driver 配置
        'spark.driver.memory': sparkConfig['spark.driver.memory'] || '2g',
        'spark.driver.cores': sparkConfig['spark.driver.cores'] || 2,
        'spark.driver.maxResultSize': sparkConfig['spark.driver.maxResultSize'] || '2g',
        
        // Spark Executor 配置
        'spark.executor.memory': sparkConfig['spark.executor.memory'] || '4g',
        'spark.executor.cores': sparkConfig['spark.executor.cores'] || 4,
        'spark.executor.instances': sparkConfig['spark.executor.instances'] || 8,
        'spark.executor.memoryFraction': sparkConfig['spark.executor.memoryFraction'] || 0.8,
        
        // 其他配置
        'spark.sql.adaptive.enabled': sparkConfig['spark.sql.adaptive.enabled'] ?? true,
        'spark.sql.adaptive.coalescePartitions.enabled': sparkConfig['spark.sql.adaptive.coalescePartitions.enabled'] ?? true,
        'spark.sql.adaptive.coalescePartitions.minPartitionNum': sparkConfig['spark.sql.adaptive.coalescePartitions.minPartitionNum'] || 1,
        'spark.sql.adaptive.coalescePartitions.initialPartitionNum': sparkConfig['spark.sql.adaptive.coalescePartitions.initialPartitionNum'] || 200,
        'spark.serializer': sparkConfig['spark.serializer'] || 'org.apache.spark.serializer.KryoSerializer',
        'spark.sql.hive.convertMetastoreParquet': sparkConfig['spark.sql.hive.convertMetastoreParquet'] ?? false,
        'spark.sql.parquet.filterPushdown': sparkConfig['spark.sql.parquet.filterPushdown'] ?? true,
        'spark.sql.parquet.mergeSchema': sparkConfig['spark.sql.parquet.mergeSchema'] ?? false,
      });
    }
  }, [visible, record, form]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateApplication(record.id, request);
    } catch (error) {
