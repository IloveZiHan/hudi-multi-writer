...
import cn.com.multi_writer.schema.OggKafkaSchema
...
/**
 * OGG Kafka数据源封装类
 * 负责消费Kafka数据并解析OGG JSON格式的binlog消息
 */
class OggKafkaSource(spark: SparkSession) {
    ...
    def createKafkaStream(): DataFrame = {
        ...
        val kafkaTopic = conf.get("spark.kafka.topic", "realtime_dataware")
    ...
    def parseKafkaMessages(kafkaStream: DataFrame): DataFrame = {
        ...
        kafkaStream
            ...
            .withColumn("parsed_data", from_json(col("kafka_value"), OggKafkaSchema.oggBinlogSchema))
            .select(
                col("kafka_key"),
                col("kafka_value"),
                col("kafka_topic"),
